...
class NewModelWithFineTuneLayer(pl.LightningModule):
    def __init__(self, pretrained_model):
        super(NewModelWithFineTuneLayer, self).__init__()
        self.pretrained_model = pretrained_model
        # Assume the pretrained model outputs features of size 128
        self.fine_tune_layer = nn.Linear(128, 10)  # Example: 10 classes

    def forward(self, x):
        x = self.pretrained_model(x)
        x = self.fine_tune_layer(x)
        return x
...
def cli_main(args):
    ...
    mask = create_mask_for_mask_type(
        args.mask_type, [0.4], args.accelerations, args.center_numbers
    )
    ...
    # import torch
    # checkpoint = torch.load('/home/qitam/sdb2/home/qiteam_project/huang/PromptMR-main/promptmr-12cascades-epoch=11-step=258576.ckpt')
    # model.load_state_dict(checkpoint['state_dict'])
    #
    # # Freeze all layers of the base model
    # for param in model.parameters():
    #     param.requires_grad = False
    # model = NewModelWithFineTuneLayer(pretrained_model)
    if args.mode == "train":
...
def build_args():
    ...
    data_path = pathlib.Path('/home/qitam/sdc1/MICCAI/home2/Raw_data/MICCAIChallenge2024/ChallengeData/MultiCoil')
