...
all_flair = glob.glob("/home/qitam/sdb2/home/qiteam_project/huang/SSL4MIS-master/data/BraTS2019/Dataset303_Falir/c/*t2f.nii.gz")
for p in all_flair:
    data = sitk.GetArrayFromImage(sitk.ReadImage(p))
    # lab = sitk.GetArrayFromImage(sitk.ReadImage(p.replace("flair", "seg")))
    lab = sitk.GetArrayFromImage(sitk.ReadImage(p.replace("t2f", "seg")))
