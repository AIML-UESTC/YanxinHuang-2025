...
class BraTS2019(Dataset):
    ...
    def __init__(self, base_dir=None, split='train', num=None, transform=None,mod = None):
        ...
        self.image_list = [item.replace('\n', '').split(",")[0] for item in self.image_list[0:300]]
        self.unlabel_image_list = [item.replace('\n', '').split(",")[0] for item in self.image_list[300:1000]]
        if num is not None:
    ...
    def __getitem__(self, idx):
        ...
        if self.mod != 'multi' and self.mod != 'multi_concat':
            ...
            sample = {'image': image, 'label': sample_1['label']}
