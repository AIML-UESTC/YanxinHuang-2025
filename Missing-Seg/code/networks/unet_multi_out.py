...
class multi_Encoder(nn.Module):
    ...
    def forward(self, x):
        conv1 = self.conv1(x.unsqueeze(1))
...
class shared_Decoder(nn.Module):
    ...
    def forward(self, center,conv4,conv3,conv2,conv1):
        ...
        return final
class shared_Decoder_con(nn.Module):
    def __init__(self, feature_scale=4, n_classes=21, is_deconv=True, in_channels=3, is_batchnorm=True):
        super(shared_Decoder_con, self).__init__()
        # upsampling
        self.is_deconv = is_deconv
        self.in_channels = in_channels
        self.is_batchnorm = is_batchnorm
    ...
    def forward(self, center,conv4,conv3,conv2,conv1):
        ...
        return final
class unet_multi_out(nn.Module):

    def __init__(self, feature_scale=4, n_classes=21, is_deconv=True, in_channels=3, is_batchnorm=True):
        super(unet_multi_out, self).__init__()
        self.is_deconv = is_deconv
        self.in_channels = in_channels
        self.is_batchnorm = is_batchnorm
        self.feature_scale = feature_scale
