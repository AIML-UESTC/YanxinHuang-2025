...
class BasicUNetDe(nn.Module):
    ...
    def __init__(
        ...
        # self.conv_0 = TwoConv(spatial_dims, 5, features[0], act, norm, bias, dropout)
        ...
        # self.final_conv = Conv["conv", spatial_dims](fea[5], 4, kernel_size=1)
    ...
    def forward(self, x: torch.Tensor, t, embeddings=None, image=None):
        ...
        if embeddings is not None:
            x0 +=embeddings[0]
            # x0 += embeddings[0][:,0:64,:,:,:]+embeddings[0][:,64:128,:,:,:]+embeddings[0][:,128:192,:,:,:]+embeddings[0][:,192:256,:,:,:]

        x1 = self.down_1(x0, temb)
        if embeddings is not None:
            x1 += embeddings[1]
            # x1 += embeddings[1][:, 0:64, :, :, :] + embeddings[1][:, 64:128, :, :, :] + embeddings[1][:, 128:192, :, :,:] + embeddings[1][:, 192:256,:, :, :]
        x2 = self.down_2(x1, temb)
        if embeddings is not None:
            x2 += embeddings[2]
            # x2 += embeddings[2][:, 0:128, :, :, :] + embeddings[2][:,128:256, :, :, :] + embeddings[2][:, 256:384, :, :,:] + embeddings[2][:, 384:512,:, :, :]

        x3 = self.down_3(x2, temb)
        if embeddings is not None:
            x3 += embeddings[3]
            # x3 += embeddings[3][:, 0:256, :, :, :] + embeddings[3][:, 256:512, :, :, :] + embeddings[3][:, 512:768, :, :,:] + embeddings[3][:, 768:1024,:, :, :]

        x4 = self.down_4(x3, temb)
        if embeddings is not None:
            x4 += embeddings[4]
            # x4 += embeddings[4][:, 0:512, :, :, :] + embeddings[4][:,512:1024, :, :, :] + embeddings[4][:, 1024:1536, :, :,:] + embeddings[4][:, 1536:2048,:, :, :]
        # 引入drop层
        x4 = self.dropout1(x4)
