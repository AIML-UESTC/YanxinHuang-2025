...
parser.add_argument('--exp', type=str,
                    default='BraTs2019_Fully_Supervised_consistency', help='experiment_name')
parser.add_argument('--mod', type=str,
                    default='multi_concat', help='experiment_mod')
parser.add_argument('--model', type=str,
                    default='unet_multi_out', help='model_name')
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            for i in range(4):
                ...
                decoded = decoder(encoded[0],encoded[1],encoded[2],encoded[3],encoded[4])
                decoded_soft = torch.softmax(decoded, dim=1)
                loss_dice = dice_loss(decoded_soft, label_batch.unsqueeze(1))+loss_dice
                loss_ce = ce_loss(decoded_soft, label_batch)+loss_ce
                loss = 0.5 * (loss_dice + loss_ce)+loss
                if i == 0:
                    mask['T1'] = decoded_soft
                elif i == 1:
                    mask['T1ce'] = decoded_soft
                elif i == 2:
                    mask['T2'] = decoded_soft
                elif i == 3:
                    mask['T2f'] = decoded_soft
            con_loss = (mse_loss(mask['T1'], mask['T2']) + mse_loss(mask['T1'], mask['T2f']) + mse_loss(
                mask['T1ce'], mask['T2']) + mse_loss(mask['T1ce'], mask['T2f']))
            consistency_loss = (loss+con_loss)/4
            # 清零所有编码器和解码器的梯度
            for i in range(4):
            ...
            writer.add_scalar('info/loss_ce', loss_ce/4, iter_num)
            writer.add_scalar('info/loss_dice', loss_dice/4, iter_num)
            # logging.info(
            #     'iteration %d : loss : %f, loss_ce: %f, loss_dice: %f' %
            #     (iter_num, loss.item(), loss_ce.item(), loss_dice.item()))
            logging.info(
                'iteration %d : total_loss : %f, loss_ce: %f, loss_dice: %f, loss_mse: %f' %
                (iter_num, consistency_loss.item(), loss_ce.item()/4, loss_dice.item()/4,con_loss.item()/4))
