...
parser.add_argument('--exp', type=str,
                    default='BraTs2019_add_fuses', help='experiment_name')
...
def dice_loss_1(pred, target, smooth=1e-6):
    # 计算 Dice 系数
...
def train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            noisy_volume_batch = volume_batch.clone()
            noisy_volume_batch[:,0,:,:,:] = T1+0.01*T1ce+0.01*T2+0.01*T2f
            noisy_volume_batch[:,1,:,:,:] = 0.01*T1+T1ce+0.01*T2+0.01*T2f
            noisy_volume_batch[:,2,:,:,:] = 0.01*T1+0.01*T1ce+T2+0.01*T2f
            noisy_volume_batch[:,3,:,:,:] = 0.01*T1+0.01*T1ce+0.01*T2+T2f
            ...
            con_loss = dice_loss_1(outputs_soft, ema_output_soft)
            con_loss_ce = ce_loss(outputs, ema_output_soft)
            loss = 0.5 * (loss_dice + loss_ce) + 0.5 * (con_loss + con_loss_ce)
            ...
            writer.add_scalar('info/loss_con_ce', con_loss_ce, iter_num)

            logging.info(
                'iteration %d : loss : %f, loss_ce: %f, loss_dice: %f, loss_con: %f, loss_con_ce: %f' %
                (iter_num, loss.item(), loss_ce.item(), loss_dice.item(),con_loss.item(),con_loss_ce.item()))
            ...
            if iter_num>3000 and iter_num % 200 == 0 :
                model.eval()
