...
parser.add_argument('--exp', type=str,
                    default='2023_copy_paste_core_region', help='experiment_name')
...
def train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            noisy_volume_batch = get_cp_to_supply(volume_batch, label_batch)

            # noisy_volume_batch = bidierction(volume_batch, label_batch,img_mask)
            ...
            # with torch.no_grad():
            pseudo_output = model(noisy_volume_batch)
            pseudo_output_soft = torch.softmax(pseudo_output, dim=1)
            con_loss = dice_loss(pseudo_output_soft,label_batch.unsqueeze(1))
            con_loss_ce = ce_loss(pseudo_output_soft,label_batch)
